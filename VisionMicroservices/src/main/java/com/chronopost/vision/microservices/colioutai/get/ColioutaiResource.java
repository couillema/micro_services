...
public class ColioutaiResource {
	...
	public Response findInfoLT(@ApiParam(required=true, value="Numéro de LT", defaultValue="EE000000001FR") @PathParam("noLT") String noLT) {
	...
	notes = "Cette version du service permet d'avoir un état de la tournée à une heure précise, et ainsi de remonter dans son déroulement.<br/>"
			...
			+ "Les colis n'ayant pas de géocodage sont géocodés via POI, puis en cas d'échec via l'API Google Maps.<br/>"
			+ "Si le colis n'a pas d'événement D+, l'ETA prévisionnel est recalculé via PTV d'après l'heure et la position de la dernière livraison.")
	public Response findInfoLT(@ApiParam(required=true, value="Numéro de LT", defaultValue="EE000000001FR") @PathParam("noLT") String noLT, @ApiParam(required=true, value="Heure au format HH:mm", defaultValue="10:00") @PathParam("heure") String heure) {
	...
	notes = "Cette version du service est utilisée pour les tests d'acceptation et les simulations, et prend en paramètre une string XML décrivant la tournée.<br/>"
			...
			+ "Les colis n'ayant pas de géocodage sont géocodés via POI, puis en cas d'échec via l'API Google Maps.<br/>"
			+ "Si le colis n'a pas d'événement D+, l'ETA prévisionnel est recalculé via PTV d'après l'heure et la position de la dernière livraison.")
	public Response findInfoLT(@ApiParam(required=true, value="Numéro de LT", defaultValue="EE000000001FR") @PathParam("noLT") String noLT, @ApiParam(required=true, value="Heure au format HH:mm", defaultValue="10:00") @PathParam("heure") String heure,@ApiParam(required=true, value="Doc XML décrivant la tournée") @PathParam("mockTempsTrajets") String mockTempsTrajets) {
