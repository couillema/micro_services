...
public class ColioutaiServiceImplTest {
	...
	final GoogleGeocoderHelper mockGoogleHelper = Mockito.mock(GoogleGeocoderHelper.class);
	
	final PoiGeocoderHelper mockPoiHelper = Mockito.mock(PoiGeocoderHelper.class);
	
	final GetLtV1 mockGetLTV1 = Mockito.mock(GetLtV1.class);
	
	final GetDetailTourneeV1 mockDetailTourneeV1 = Mockito.mock(GetDetailTourneeV1.class);
	
	final GetCodeTourneeFromLtV1 mockCodeTourneeV1 = Mockito.mock(GetCodeTourneeFromLtV1.class);
	
	final ConsigneServiceWS mockConsigneServiceWS = Mockito.mock(ConsigneServiceWS.class);
	
	private ColioutaiServiceImpl service = new ColioutaiServiceImpl(mockGoogleHelper, mockPoiHelper, 
			mockGetLTV1, mockDetailTourneeV1, mockCodeTourneeV1, mockConsigneServiceWS);
	
	private void resetMocks() {
		Mockito.reset(mockGoogleHelper);
		Mockito.reset(mockPoiHelper);
		Mockito.reset(mockGetLTV1);
		Mockito.reset(mockDetailTourneeV1);
		Mockito.reset(mockCodeTourneeV1);
		Mockito.reset(mockConsigneServiceWS);
	}
	...
	public void testFoundNotFound() throws Exception {
		...
		final ColioutaiInfoLT infoLT = service.findInfoLT("1234", DateRules.toTodayTime("09:00"), null);
		...
		assertEquals(infoLT.getPositionTournee(), new Integer(12));
	...
	private void checkLT(ColioutaiInfoLT infoLT, int number) {
		...
		GeoAdresse adresse = new GeoAdresse(null, null, lt.getAdresse1Destinataire(), lt.getAdresse2Destinataire(), lt.getCodePostalDestinataire(), lt.getVilleDestinataire());
	...
	public void testRecupCoordonneesFromGeoCoding() throws Exception {		
		...
		Mockito.when(mockGoogleHelper.geocodeFrom(new GeoAdresse(null, null, "80 Avenue d'Estienne d'Orves", null, "91260", "Juvisy sur orge"))).thenReturn(positionGoogle);
		...
		Mockito.when(mockPoiHelper.geocodeFrom(new GeoAdresse(null, null, "80 Avenue d'Estienne d'Orves", null, "91260", "Juvisy sur orge"))).thenReturn(positionPoi);
	...
	public void testHasCL() throws Exception {
		...
		Mockito.when(mockPoiHelper
				.geocodeFrom(new GeoAdresse("vivien", "de saint pern", "5 place de rungis", null, "75013", "Paris")))
	...
	public void testHasCLsurLTdansTournee() throws Exception {
		...
		// qui est résolu par POI
		Mockito.when(mockPoiHelper.geocodeFrom(new GeoAdresse("vivien", "de saint pern", "5 place de rungis", null, "75013", "Paris"))).thenReturn(positionPoi);
	...
	private List<Evt> buildEvenementListForGeoTest() throws ParseException {
		List<Evt> evtList = new ArrayList<Evt>();
		 ...
		 Map<String,String> infoscomp = new HashMap<String, String>();
	        ...
	        Evt evt = new Evt().setPrioriteEvt(146)
			  		.setDateEvt(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2015-03-18 22:26:00"))
			  		.setNoLt("EEINSEVT001FR")
			  		.setCabRecu("%0020090NA146848396248899250")
			  		.setCodeEvt("TA")
			  		.setCreateurEvt("TRI1")
			  		.setDateCreationEvt("2015-09-01T22:34:56")
			  		.setIdAccesClient(0)
			  		.setIdExtractionEvt("717493191")
			  		.setIdbcoEvt(88)
			  		.setLibelleEvt("Envoi en transit")
			  		.setLieuEvt("93999")
			  		.setPositionEvt(0)
			  		.setRefIdAbonnement("EVT_CHR")
			  		.setSsCodeEvt("AJA0")
			  		.setStatusEvt("Acheminement en cours")
			  		.setInfoscomp(infoscomp)
			  		.setCodePostalEvt("13999")
			  		.setCabEvtSaisi("cab_evt_saisi")
			  		.setCodeEvtExt("toto")
			  		.setCodeRaisonEvt("code_raison_evt")
			  		.setCodeService("code_service")
			  		.setIdSsCodeEvt(1)
			  		.setLibelleLieuEvt("libelle_lieu_evt")
			  		.setProdCabEvtSaisi(1)
			  		.setProdNoLt(1)
	...
	private void prepareDataWithCL() throws Exception {
		Map<String, Lt> mapLT_COLIS_4 = new HashMap<>();
	...
	public void testEtatPoint() {
		...
		// assertEquals(service.computeEtatPoint(infoLT).getEtatPoint(), ColioutaiInfoLT.ETAT_POINT_POSTPONED);
	}
