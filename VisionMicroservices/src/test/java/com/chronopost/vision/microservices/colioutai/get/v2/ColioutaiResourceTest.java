...
public class ColioutaiResourceTest extends JerseyTestNg.ContainerPerClassTest {
    ...
    public void testGetLTFoundNotFound() throws Exception {
        ...
        Mockito.when(serviceMock.findInfoLT(Mockito.eq("5678"), Mockito.any(Date.class), Mockito.isNull(String.class))).thenReturn(
        		colioutaiInfoLT5678);
        
        Mockito.when(serviceMock.findInfoLT(Mockito.eq("1234"), Mockito.any(Date.class), Mockito.isNull(String.class))).thenThrow(new ColioutaiException(ColioutaiException.LT_NOT_FOUND));
        
        int status = client.target("http://localhost:"+ getPort()).path("/v2/colis/1234").request()
        			.accept(MediaType.APPLICATION_JSON_TYPE).get().getStatus();
        
        assertEquals(status, 404);
       
        Response response = client.target("http://localhost:"+ getPort()).path("/v2/colis/5678").request()
